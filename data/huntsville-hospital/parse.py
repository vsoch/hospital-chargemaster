...
for result in results:
    ...
    if filename.endswith('csv'):
        ...
        if charge_type == "drg":
            ...
            label = "Average Case"
            if label not in content.columns.tolist():
                label = 'Helen Keller Avg. Case'

            # ['Category', 'MSDRG', 'MSDRG Description', 'Average Case']
            for row in content.iterrows():
                idx = df.shape[0] + 1
                if pandas.isnull(row[1][label]):
                    continue
                price = row[1][label].replace('$','').replace(',','').strip()
                entry = [row[1]['MSDRG'],               # charge code
                         price,                         # price
                         row[1]['MSDRG Description'],   # description
