...
for result in results:
    ...
    if filename.endswith('xlsx'):
        ...
        if charge_type == "standard":
            ...
            # Thank you St. Luke's for this beautiful organized data!
            # ['PROCEDURE RECORD NAME', 'UNIT CHARGE', 'Comment']
            content = pandas.read_excel(filename)

            for row in content.iterrows():
                idx = df.shape[0] + 1
                entry = [None,                            # charge code
                         row[1]['UNIT CHARGE'],           # price
                         row[1]['PROCEDURE RECORD NAME'], # description
