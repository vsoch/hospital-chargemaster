...
for result in results:
    ...
    if filename.endswith('csv'):
        ...
        # ['Number', 'General Description', 'Charge']
        if charge_type == 'standard':
            ...
            for row in content.iterrows():
                ...
                entry = [row[1]['Number'],                     # charge code
                         row[1]['Charge'].replace('$','').replace(',','').strip(),  # price
                         row[1]['General Description'],        # description
                         result["hospital_id"],        # hospital_id
                         result['filename'],
                         charge_type]            
                df.loc[idx,:] = entry

        # ['Inpatient DRG', 'Avg Charges per DRG']
        else:
            content = pandas.read_csv(filename, skiprows=3)
            for row in content.iterrows():
                idx = df.shape[0] + 1
                if pandas.isnull(row[1]['Inpatient DRG']):
                    continue
                code = row[1]['Inpatient DRG'].split('-')[0]
                entry = [code,                         # charge code
                         row[1]['Avg Charges per DRG'].replace('$','').replace(',','').strip(),  # price
                         row[1]['Inpatient DRG'],        # description
                         result["hospital_id"],        # hospital_id
                         result['filename'],
                         charge_type]
                df.loc[idx,:] = entry
