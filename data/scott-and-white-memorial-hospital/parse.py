...
for result in results:
    ...
    if filename.endswith('xlsx'):
        content = pandas.read_excel(filename, 1)
        
        #['Charge Code', 'Charge Code Description', 'Lowest Price', 'Highest Price', '**Price Data is effective as of 12/4/2018']

        for row in content.iterrows():
            idx = df.shape[0] + 1
            if "Price" not in row[1]:
                price = (row[1]['Highest Price'] + row[1]['Lowest Price']) / 2
            else:
                price = row[1]['Price']
            entry = [row[1]['Charge Code'],                    # charge code
                     price,                          # price
                     row[1]['Charge Code Description'],        # description
