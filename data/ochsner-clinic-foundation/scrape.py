...
url = 'https://www.ochsner.org/patients-visitors/billing-and-financial-services/estimates'
...
for entry in soup.find_all('a', href=True):
    page_url = entry['href']
    if 'billing-estimates' in page_url:  
        hospital_id = page_url.split('/')[-1].replace('billing-estimates-ochsner-','')
        hospital_page = requests.get(page_url)
        hospital_soup = BeautifulSoup(hospital_page.text, 'lxml')
        for item in hospital_soup.find_all('a', href=True):
