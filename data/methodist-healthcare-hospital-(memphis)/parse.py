...
for result in results:
    ...
    # The other files are inconsistently formatted with various hospitals
    # we will just parse the chargemaster file
    if filename.endswith('xlsx'):
        if "chargemaster" not in os.path.basename(filename).lower():
            continue

    # ['FACILITY', 'CPT4', 'CPT4 SHORT DESCRIPTION', 'MLH 2019 CHG']
    content = pandas.read_excel(filename)
    for row in content.iterrows():
        idx = df.shape[0] + 1
        entry = [row[1]['CPT4'],                      # charge code
                 row[1]['MLH 2019 CHG'],              # price
                 row[1]['CPT4 SHORT DESCRIPTION'],    # description
