    ...
    if filename.endswith('csv'):
        ...
        if charge_type == "standard":
            ...
            for row in content.iterrows():
                idx = df.shape[0] + 1
                if pandas.isnull(row[1][' CurrPrice ']):
                    continue
                entry = [None,                         # charge code
                         row[1][' CurrPrice '].replace('$','').replace(',','').strip(),        # price
            ...
            # ['MS DRG', 'Description', 'Cases', 'Total Charges', 'Avg Charge']
            content = pandas.read_csv(filename, skiprows=4)
            if "MS DRG" not in content.columns:
                content = pandas.read_csv(filename, skiprows=5)
            for row in content.iterrows():
                idx = df.shape[0] + 1
                entry = [row[1]['MS DRG'],                                             # charge code
                         row[1]['Avg Charge'].replace('$','').replace(',','').strip(), # price
                         row[1]['Description'],        # description
                         result["hospital_id"],        # hospital_id
                         result['filename'],
                         charge_type]            
