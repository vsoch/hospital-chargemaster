...
for result in results:
    ...
    if filename.endswith('xlsx'):
        ...
        # ['Unnamed: 0', 'Facility Name', 'Activity Code / Medication ID / MS-DRG',
        # 'Item/Service Description or Medication Name',
        # 'Charge Price or Average Charge Price']
        content = pandas.read_excel(filename, skiprows=4)

        for row in content.iterrows():
            idx = df.shape[0] + 1
            entry = [row[1]['Activity Code / Medication ID / MS-DRG'],  # charge code
                     row[1]['Charge Price or Average Charge Price'],             # price
                     row[1]['Item/Service Description or Medication Name'],        # description
