...
for result in results:
    ...
    if filename.endswith('txt'):
        ...
        # No idea what encoding this was in, didnt work
        os.system('dos2unix %s' %filename)

        # ['SERVICE_CODE', 'GENERAL_DESC', '2019 Price', '2019 Average AGB']
        content = pandas.read_csv(filename, sep='\t')

        for row in content.iterrows():
            idx = df.shape[0] + 1
            entry = [row[1]['SERVICE_CODE'],         # charge code
                     row[1]['%s Price' % year],                        # price
                     row[1]['GENERAL_DESC'],    # description
